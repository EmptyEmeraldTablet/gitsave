...
- **交互式 TUI**：可视化展示路线、历史、工作区状态，并带有确认弹窗与快捷操作。
...
TUI 已经成为 gitsave 的一等公民，不再处于实验阶段。所有功能都通过 `gitsave tui` 启动，使用前请先在目标目录执行 `gitsave init`。

### 界面布局
- **顶部标题栏**：显示当前保存路径、所在路线、Autosave 状态与刷新间隔。当执行耗时操作时会展示旋转指示，并把终端光标切换为细条形，直至操作完成。
- **左侧 Routes 面板（约 40% 宽度）**：列出所有路线，`*` 表示当前 CLI/TUI 正处于该路线；面板底部的 Autosave 卡片会展示启用状态、间隔、最大保存数以及最近一次自动保存时间。
- **右侧 History 面板（约 60% 宽度）**：上半部分列出当前路线下的保存记录，下半部分的 Status 卡片展示「当前提交」「选中提交」以及工作区脏文件详情，帮助判断是否需要保存或能否安全切换路线。
- **底部 Notifications**：滚动显示最近 4 条系统消息/错误。由于确认与输入已改用中央弹窗，这里只记录事件，不会被提示语打断。
- **中央弹窗（按需出现）**：创建路线、切换路线、加载保存、快速保存等操作都会弹出确认对话框；路名字串输入时也在此完成，避免和通知打架。
...
| `q` | 退出 TUI |
| `r` | 手动刷新（最快获取 CLI 侧变更） |
| `Tab` | 在 Routes 与 History 面板之间切换焦点 |
| `↑/↓` 或 `j/k` | 移动当前焦点的光标 |
| `PgUp/PgDn` | 浏览 History 列表时快速跳页 |
| `Enter` | 在 Routes 中切换路线 / 在 History 中加载选中保存（均带确认弹窗） |
| `s` | 发起快速保存（需确认，支持脏工作区） |
| `l` | 加载当前选中的保存 |
| `c` | 创建新路线，支持 `-/_/` 等字符 |
| `a` | 立即触发自动保存检测（仅当 autosave 已启用） |

### 使用建议
- TUI 会自动检测未跟踪文件并在 Status 面板列出，切换路线/回滚时若存在脏数据会给出额外警告。
- 所有保存/加载/路线切换操作都会显示繁忙指示与加载动画，长时间 I/O（例如大体积游戏存档）时请耐心等待。
- 若 Autosave 处于关闭状态，可以在 CLI 中运行 `gitsave autosave --enable` 后回到 TUI，再按 `a` 进行一次手动检测确认配置生效。
